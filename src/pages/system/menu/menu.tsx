...
const Menu = ({permissions}: Props) => {
  ...
  const [tableData, setTableData] = useState<SysMenuItem[]>([]);
  ...
  const delRecord = async (record: SysMenuItem) => {
  ...
  const columns: ColumnsType<SysMenuItem> = [
    {
      title: '菜单名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      render(value, record, index) {
        return (<Space>
          ...
          {checkPermission(buttonPermissionKeys.delBtn, <Popconfirm title={`确定要删除【${record.name}】吗`}
                                                                    okText={'确定'}
                                                                    cancelText={'取消'}
  ...
  function editRecord(record: SysMenuItem) {
  ...
  function submitForm() {
    form.validateFields().then(async (values: SysMenuItem) => {
      const isUpdate = Boolean(values.id);
      if (isUpdate) {
        await MenuService.updateMenu(values);
      } else {
  ...
  return <Card>
    <div className={styleds.moduleHeader}>
      ...
      <div className={styleds.tableExtension}>
        {checkPermission(buttonPermissionKeys.creatBtn, <Button type={'primary'} onClick={addRecord}>新增菜单</Button>)}
        <Tooltip title={'刷新'}>
